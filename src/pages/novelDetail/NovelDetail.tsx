...
export default function NovelDetail() {
  ...
  const { loading, error, data } = useQuery(GET_DETAIL_NOVEL, {
    variables: {
  ...
  useEffect(() => {
    if (user && data) {
      const isOwner = user.userId === data?.novel?.user_id;
  ...
  const { novel } = data;
  console.log(novel?.novel_badges);
    ...
    <>
      ...
      <div className="content flex-1">
        ...
        <div className="novel-item border-t-2 border-gray-300 py-3">
              ...
              <div className="novel-right flex-1">
                ...
                <div className="flex flex-col gap-4 py-3">
                  <div className="flex ">
                    ...
                    <div className="flex-1">{novel.episodes.length}</div>
                  </div>
                  <div className="flex ">
          ...
          <div>
            <ul className="flex flex-wrap items-center gap-x-4 pt-2">
              {novel?.novel_badges.map((badges, index) => (
                <li
                ...
                <>
                  {novel.episodes.map((episode, index) => (
                    <div className="mb-4 flex gap-28 border-b-2">
                      ...
                      <div className="flex-1">{episode.episode_title}</div>
                      <div className="flex-1">
                        {episode.first_novel_publish_at > episode.updated_at
                          ? moment(episode.first_novel_publish_at).format(
                              "Y-MM-DD HH:MM:ss",
                            )
