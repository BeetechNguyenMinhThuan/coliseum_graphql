import { useTranslation } from "react-i18next";
import { SideBarColiseum } from "components/SideBar/SideBarColiseum.tsx";
import { Advertisement } from "components/Advertisement";
import "./userManual.scss";
export function UserManual() {
  const { t } = useTranslation();

  const arrAds = [
    "s-l1200.webp",
    "coke-print-ad.jpg",
    "7_Up_-_You_like_it,_it_likes_you,_1948.jpg",
  ];
  return (
    <>
      <SideBarColiseum />
      <div className="content flex-1">
        <div className="border-black-500 min-h-[188px] border-2 border-l-0 border-solid p-2 ">
          <div className="flex flex-col justify-between border-2 p-4">
            <div className="border-b-2 border-dashed pb-4  text-center text-2xl font-bold">
              作家の遊び場
            </div>
            <div className="my-3 flex">
              <img
                src="https://png.pngtree.com/thumb_back/fh260/background/20201009/pngtree-black-friday-sale-horizontal-banner-with-red-background-with-place-for-image_405375.jpg"
                alt=""
              />
              <p className="px-4">
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              </p>
            </div>
            <button className="w-full rounded-lg border-2 p-3 text-2xl font-bold">
              作家の遊び場
            </button>
            <span className="my-3 font-bold">
              指示 利用規約 ポリシー お問い合わせ 私の会社指示 利用規約 ポリシー
              お問い合わせ 私の会社指示 利用規約 ポリシー お問い{" "}
            </span>
            <hr className="my-3" />

            <div className="border-b-2 border-dashed pb-4  text-center text-2xl font-bold">
              私の会社指示 利用規約 ポリシー
            </div>
            <div className="my-3 flex">
              <img
                className="border-2"
                src="https://lh4.googleusercontent.com/a8Inr0Fg78O9k_BUPg8zrQFFK_Qr7mt5MUid6Gn2IKGuzkMlG1o4zPPCkGMP8VrNGq19NLW6Xp7UzobQsWH2vWQ=w16383"
                alt=""
              />
              <p className="px-4">
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              </p>
            </div>
            <hr className="my-3" />

            <div className="border-b-2 border-dashed pb-4  text-center text-2xl font-bold">
              私の会社指示 利用規約 ポリシー
            </div>
            <div className="my-3 flex">
              <img
                className="border-2"
                src="https://lh4.googleusercontent.com/a8Inr0Fg78O9k_BUPg8zrQFFK_Qr7mt5MUid6Gn2IKGuzkMlG1o4zPPCkGMP8VrNGq19NLW6Xp7UzobQsWH2vWQ=w16383"
                alt=""
              />
              <p className="px-4">
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              </p>
            </div>
            <hr className="my-3" />
          </div>
          <Advertisement>{arrAds}</Advertisement>
          <div className="border-2 p-4">
            <div className="my-4 border-b-2 border-dashed py-3 text-center text-2xl font-bold">
              ユーザーマニュアル
            </div>
            <span className="font-bold">1. 著者と著者に共通</span> <br />
            <span className="ml-3"> 1-1の読者。 ユーザー登録について</span>
            <br />
            <span className="font-bold">2. ご協力について フェイク</span>
            <br />
            <span className="ml-3"> 2-1。 Rubyのセットアップ方法</span>
            <hr className="my-3" />
            <div className="">
              <h2 className="text-xl font-bold">1. 作者と読者の共通点</h2>
              <div className="ml-4">
                <span className="my-4 font-bold">
                  1-1.ご協力について フェイ
                </span>
                <div className="leading-8">
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                  透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけ
                </div>
                <button className="w-full rounded-lg border-2 p-3 text-2xl font-bold">
                  作家の遊び場
                </button>
                <hr className="my-3" />
                <h2 className="text-xl font-bold">2. 作者と読者の共通点</h2>
                <div className="ml-4">
                  <span className="my-4 font-bold">
                    2-1.ご協力について フェイ
                  </span>
                  <div className="leading-8">
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆
                    透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="my-3 border-2 p-4">
            <div className="mb-4 border-b-2 border-dashed pb-3 text-center text-2xl font-bold">
              ユーザーマニュアル
            </div>
            <div className="border-b-2 px-8 pb-4">
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              <br />
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語
            </div>

            <div className="mb-4 border-b-2 border-dashed pb-3 text-center text-2xl font-bold mt-3">
              ユーザーマニュアル
            </div>
            <div className=" px-8 pb-4">
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語14拒ノカニ生人むびこ北読列トろけち。
              透ロ校数ナ舎会シ打彼優フヌヒ調26空製ケフエヨ毎逆由クじ語
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
